...
GBufferOutput2 PSMain(PSInput input) 
{
    ...
    int mainTextureId = Materials[MaterialId].TextureA;
    float4 texColor = sampleTex(mainTextureId, input.uv);
	
    float a = sampleTex(mainTextureId, input.uv + float2(+0.05, +0.05)).a;
